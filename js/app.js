...
function seed() {
  return { settings: { theme: 'light', seq: 1, capitalInvestido: 0, percentualJuros: 50 }, clientes: [], vales: [] };
}
...
function normalizeDb(obj) {
  ...
  obj.settings.percentualJuros = Number(obj.settings.percentualJuros || 50);
...
function renderDashboard() {
  ...
  const percentualJuros = Number(db.settings.percentualJuros || 50);
  const valorEmprestado = aberto.reduce((s, v) => s + Number(v.valor || 0), 0);
  const totalJuros = aberto.reduce((s, v) => s + Math.max(0, Number(v.total || 0) - Number(v.valor || 0)), 0);
  const jurosPercentual = totalJuros * (percentualJuros / 100);
  ...
  if ($('configPercentualJuros') && document.activeElement !== $('configPercentualJuros')) $('configPercentualJuros').value = String(percentualJuros).replace('.', ',') + '%';
  if ($('dashCaixa')) $('dashCaixa').textContent = money(valorEmCaixa);
  if ($('dashInvestido')) $('dashInvestido').textContent = money(capitalInvestido);
  $('dashEmprestado').textContent = money(valorEmprestado);
...
function saveDashboardConfig() {
  ...
  db.settings.percentualJuros = taxaNum($('configPercentualJuros').value);
  save();
  renderDashboard();
