...
function renderDashboard() {
  ...
  const totalRecebido = pagos.reduce((s, v) => {
    const principal = Number(v.valor || 0);
    const totalPago = Number(v.total || 0);
    return s + Math.max(0, totalPago - principal);
  }, 0);
...
function renderAll() { renderClientOptions(); renderClients(); renderHistory(); renderDashboard(); renderNotifications(); renderReports(); }
